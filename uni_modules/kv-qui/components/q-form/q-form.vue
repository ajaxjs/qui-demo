<template>
	<form ref="rootRef" class="q-form" @submit="submit" @reset="reset">
...
<script>
	import { ref, onActivated, onDeactivated, onMounted, getCurrentInstance, nextTick, provide } from 'vue'
				...
				validate().then(val => {
					...
					if (index === validateIndex && val === true) {
						if (props.onSubmit !== void 0) {
							emit('submit', evt)
						} else if (evt !== void 0 && evt.target !== void 0 && typeof evt.target.submit ===
							'function') {
							evt.target.submit()
						}
					}
			...
			function focus() {
				addFocusFn(() => {
					...
					const target = rootRef.value.querySelector(
						...
						Array.prototype.find.call(rootRef.value.querySelectorAll('[tabindex]'), el => el
							.tabIndex > -1)
