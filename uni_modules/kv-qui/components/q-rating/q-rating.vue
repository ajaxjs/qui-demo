<template>
	<view :class="classes" :style="sizeStyle">
		<view v-for="(vo, i) in stars" :key="i" class="q-rating__icon-container flex flex-center"
			@click="set(i+1)"
			@mouseover="setHoverValue(i+1)"
			@mouseout="resetMouseModel"
		>
			<slot :name="'tip-'+(i+1)"></slot>
			<q-icon :name="vo.name" :class="vo.iconClass" />
		</view>
	</view>
</template>
		...
		setup(props, { slots, emit }) {
			...
			const iconData = computed(() => {
				const iconLen = Array.isArray(props.icon) === true ? props.icon.length : 0,
					selIconLen = Array.isArray(props.iconSelected) === true ? props.iconSelected.length :
					0,
					...
					selColorLen = Array.isArray(props.colorSelected) === true ? props.colorSelected
					.length : 0,
			...
			const stars = computed(() => {
				...
				for (let i = 1; i <= props.max; i++) {
					const
						active = (mouseModel.value === 0 && props.modelValue >= i) || (mouseModel.value >
							0 && mouseModel.value >= i),
						half = halfIndex === i && mouseModel.value < i,
						exSelected = mouseModel.value > 0 && (half === true ? ceil : props.modelValue) >=
						i && mouseModel.value < i,
						color = half === true ?
					...
					acc.push({
						name: (
							...
							(i <= icons.halfIconLen ? props.iconHalf[i - 1] : icons.halfIcon) :
							(
								icons.selIcon !== void 0 && (active === true || exSelected ===
									true) ?
								(i <= icons.selIconLen ? props.iconSelected[i - 1] : icons
									.selIcon) :
								(i <= icons.iconLen ? props.icon[i - 1] : icons.icon)
							)
						) || 'star'/*$q.iconSet.rating.icon*/,
